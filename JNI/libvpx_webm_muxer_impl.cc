...
#define FUNC(RETURN_TYPE, NAME, ...) \
  extern "C" { \
  JNIEXPORT RETURN_TYPE Java_com_google_libvpx_WebmWriter_ ## NAME \
...
FUNC(jlong, vpxCodecWebmAllocCfg) {
  printf("vpxCodecWebmAllocCfg");
...
FUNC(void, vpxCodecWebmFreeCfg, jlong jglob) {
  printf("vpxCodecWebmFreeCfg");
...
void Ebml_Write(EbmlGlobal *glob, const void *buffer_in, uint64_t len) {
...
FUNC(void, vpxCodecWebmWriteWebmFileHeader, jlong jglob,
                                            jint width, jint height,
                                            jobject jfps) {
  printf("vpxCodecWebmWriteWebmFileHeader");
  EbmlGlobal *glob = reinterpret_cast<EbmlGlobal*>(jglob);
  ...
  jmethodID rationalGetNum = env->GetMethodID(rational, "num", "()J");
  assert(rationalGetNum != NULL);

  jmethodID rationalGetDen = env->GetMethodID(rational, "den", "()J");
  assert(rationalGetDen != NULL);

  jmethodID rationalToFloat = env->GetMethodID(rational, "toFloat", "()F");
  assert(rationalToFloat != NULL);
...

  {
    ...
    glob->framerate.num = env->CallLongMethod(jfps, rationalGetNum, rational);
    glob->framerate.den = env->CallLongMethod(jfps, rationalGetDen, rational);
...

    {
      ...
      Ebml_StartSubElement(glob, &trackStart, Tracks);
      {
        ...
        Ebml_SerializeString(glob, CodecID, "V_VP8");
        {
          ...
          float        frameRate   = env->CallFloatMethod(jfps, rationalToFloat, rational);
...
FUNC(void, vpxCodecWebmWriteWebmBlock, jlong jglob, jobject jpkt, jlong jpts) {
  printf("vpxCodecWebmWriteWebmBlock");
  EbmlGlobal *glob = reinterpret_cast<EbmlGlobal*>(jglob);
  ...
  jfieldID szId = env->GetFieldID(codecCxPkt, "sz", "J");
  assert(szId != NULL);

  jobject jba = env->GetObjectField(jpkt, bufferId);
  assert(jba != NULL);

  jint frameFlags = env->GetIntField(jpkt, flagsId);
  ...
  jlong frameSz =  env->GetLongField(jpkt, szId);
...
FUNC(void, vpxCodecWebmWriteWebmFileFooter, jlong jglob, jlong hash) {
  printf("vpxCodecWebmWriteWebmFileFooter");
  EbmlGlobal *glob = reinterpret_cast<EbmlGlobal*>(jglob);
